...
def _segment_reduction_fn(data, segment_ids, reduction, num_segments):
    ...
    if reduction == "amax":
        reduce_op = paddle.max
        initial_value = -float("Inf")
    elif reduction == "amin":
        reduce_op = paddle.min
        initial_value = float("Inf")
    elif reduction == "prod":
        reduce_op = paddle.prod
        initial_value = 1.0
    else:
        reduce_op = paddle.sum
        initial_value = 0.0
    ...
    for i in range(num_segments):
        ...
        if paddle.any(mask).item():
            outputs.append(reduce_op(flat_data[mask], axis=0))
        else:
            outputs.append(
...
def top_k(x, k, sorted=True, is_stable=True):
    ...
    sorted_indices = paddle.argsort(x, axis=-1, descending=True, stable=True)
    top_k_indices = sorted_indices[..., :k]
    return paddle.take_along_axis(x, top_k_indices, axis=-1), top_k_indices
...
def istft(
    ...
    if win is not None:
        ...
        from keras_paddle.src.ops.numpy import pad as _pad
